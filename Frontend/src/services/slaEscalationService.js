...
const checkSLAEscalations = async () => {
  try {
    ...
    for (const ticket of activeTickets) {
      ...
      const escalationLimitMs = convertToMilliseconds(
        policy.escalationTimeValue,
        policy.escalationTimeUnit
      );

      if (!escalationLimitMs) {
        continue;
      }

      const ticketAgeMs = now - new Date(ticket.createdAt);
      const escalationBreached = ticketAgeMs > escalationLimitMs;

      if (escalationBreached) {
        ticket.status = 'ESCALATED';
        ticket.slaPolicy = policy._id;
        ticket.autoEscalated = true;
        ticket.escalatedAt = new Date();
        ticket.escalationReason = 'SLA escalation time breached';
